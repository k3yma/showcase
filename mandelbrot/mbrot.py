...
def get_colour(col_val):
    cols=['#421e0f','#250726','#19071a','#09012f','#040449','#000764','#0c2c8a','#1855b1','#397dd1',
          '#86b5e5','#d3ecf8','#f1e9bf','#f8c95f','#ffaa00','#cc8000','#995700','#6a3403']
    q=col_val%len(cols)
...
def get_array():
    return np.array([[calc_value(complex(minx+(x*(maxx-minx)/resolution),miny+(y*(maxy-miny)/resolution))) for x in range(resolution)] for y in range(resolution)])
...
def button_pressed(event):
    ...
    rect = c.create_rectangle(rect_draw_x,rect_draw_y,rect_draw_x,rect_draw_y,outline="red")
...
def button_released(event):
    c.delete(rect)
    redefine_scale(min(rect_draw_x1, rect_draw_x), max(rect_draw_x1, rect_draw_x), min(rect_draw_y1, rect_draw_y), max(rect_draw_y1, rect_draw_y))
    draw_mbrot()
