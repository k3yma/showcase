    ...
    for r in range(3):
        for c in range(3):
            if dice[val_normalized][r * 3 + c] == "x":
                ...
                dots.ellipse(
                    (x - dot_size / 2, y - dot_size / 2, x + dot_size, y + dot_size),
                    fill=dot_col,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-x", default="100", help="Maximum Width in Dice", metavar="MaxWidth"
    )
    parser.add_argument(
        "-y", default="100", help="Maximum Height in Dice", metavar="MaxHeight"
    )
    ...
    for y in range(img.height):
        for x in range(img.width):
            img_new.paste(
                get_dice(img.getpixel((x, y)), dice_size),
                (x * dice_size, y * dice_size),
            )
